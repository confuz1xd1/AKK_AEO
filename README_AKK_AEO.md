Ниже структура текста под курсовую/отчет именно по той модели: простая полносвязная сеть на TensorFlow, обученная на русском датасете `k1tub/sentiment_dataset` (отзывы/комментарии с метками негатив/нейтрал/позитив) как прокси для студентческих эссе и отзывов о лекциях.
